...
这里, 我们接着公式 $(4.3)$ 的假设往下走。假设在训练阶段的某一次迭代中, 模型输入的中心词都是 $w$。我们从数据集概率分布 $P_d(C|w)$ 中采样 $k_d$ 个样本, 从噪声分布 $P_n(C|w)$ 中采样 $k_n$ 个样本。噪声分布 是我们预先设定好的。NCE 要做的事情就是判断 $c$ 标签是从数据集 $P_d(C|w)$ 中采样出来的概率大还是从噪声分布 $P_n(C|w)$ 中采样出来的概率大。

我们用 $D=1$ 表示 $c$ 从数据集中采样出来的, $D=0$ 表示 $c$ 从噪声分布中采样出来的, 假设 $k = \frac{k_n}{k_d}$, 即从 噪声分布 中采样出来 $c$ 的数量是从 数据集 中采样出来的 $k$ 倍。那么, 我们可以得到:
